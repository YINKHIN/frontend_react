...
const ImportModal = ({ importItem, mode, onClose, onSaved }) => {
    ...
    if (importItem) {
      try {
        ...
        if (importItem.importDetails && importItem.importDetails.length > 0) {
          detailsToLoad = importItem.importDetails;
        } else if (importItem.import_details && importItem.import_details.length > 0) {
          detailsToLoad = importItem.import_details;
        }
        
        if (detailsToLoad.length > 0) {
          const formattedDetails = detailsToLoad.map(detail => ({
        ...
        console.error('Error loading import item:', error);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      // Call the onSaved callback if provided, otherwise use onClose
      if (onSaved) {
        onSaved();
      } else {
        onClose();
      ...
      const errorMessage = error.response?.data?.message || error.message || 'Failed to save import';
  ...
  const ProductForm = ({ products, onAdd, onCancel }) => {
    ...
    const handleFormSubmit = (data) => {
      ...
      console.log('Adding product:', data); // Debug log
      onAdd(data);
      productReset();
    };
