...
const StaffPage = () => {
  ...
  const { data: staffsResponse, isLoading, error } = useStaffs()
            ...
            <tbody>
              {filteredStaffs.map((staff) => (
                <tr key={staff.id} className="table-row">
                    ...
                    <div className="w-16 h-16 bg-gray-100 rounded-lg flex items-center justify-center overflow-hidden border border-gray-200">
                      {(staff.photo_url || staff.photo) ? (
                        <img
                          ...
                          onError={(e) => {
                            console.log('Staff image failed to load:', e.target.src)
                      ...
                      <UserCheck className={`fallback-icon w-8 h-8 text-gray-400 ${(staff.photo_url || staff.photo) ? 'hidden' : ''}`} />
                    </div>
                  </td>
                  <td className="table-cell">
                    <div>
                      <div className="font-medium text-gray-900">{staff.full_name}</div>
                      <div className="text-sm text-gray-500">{staff.dob}</div>
                    </div>
                  </td>
                  <td className="table-cell">
                  ...
                  <td className="table-cell">
                    <span className="text-gray-900">{staff.gen === 'M' ? 'Male' : 'Female'}</span>
                  </td>
                  <td className="table-cell">
                    <span className="font-medium text-gray-900">${staff.salary}</span>
                  </td>
                  <td className="table-cell">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${staff.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                      {staff.status}
                    </span>
                  </td>
                  <td className="table-cell">
